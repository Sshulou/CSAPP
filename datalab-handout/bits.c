...
int isLessOrEqual(int x, int y) {
  ...
  return (signsDiffer & xSign) | ((!signsDiffer) & differenceNonnegative);
}
//4
/* 
...
int howManyBits(int x) {
  ...
  return bits16 + bits8 + bits4 + bits2 + bits1 + lastBit + 1;
}
//float
/* 
...
int floatFloat2Int(unsigned uf) {
  ...
  if (exponent > 23) {
    value = fraction << (exponent + (~23 + 1));
  } else {
    value = fraction >> (23 + (~exponent + 1));
  }
